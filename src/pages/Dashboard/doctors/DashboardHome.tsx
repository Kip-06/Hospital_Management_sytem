...
const DashboardPage: React.FC = () => {
  ...
  const { data: dashboardStats, isLoading: statsLoading } = dashboardApi.useGetDashboardStatsQuery(
    doctorId ? { userType: 'doctor', userId: doctorId } : skipToken
  );
  const { data: activities, isLoading: activitiesLoading } = dashboardApi.useGetRecentActivitiesQuery({ limit: 4 });
  const { data: todayAppointments, isLoading: appointmentsLoading } = dashboardApi.useGetTodaysAppointmentsQuery({ 
    userType: 'doctor', 
    userId: doctorId 
  });
  const { data: patientUpdates, isLoading: updatesLoading } = dashboardApi.useGetPatientUpdatesQuery({ 
    limit: 3,
    doctorId 
  });
  const { data: weeklySchedule, isLoading: scheduleLoading } = dashboardApi.useGetWeeklyScheduleQuery({ 
    userType: 'doctor', 
    userId: doctorId 
  });
  ...
  const handleViewAllUpdates = useCallback(() => {
    navigate('/doctor/patient-updates');
  }, [navigate]);

  const handleViewFullCalendar = useCallback(() => {
    navigate('/doctor/doctor-calendar');
  }, [navigate]);

  const handleViewAllActivities = useCallback(() => {
    navigate('/doctor/activities');
  }, [navigate]);

  const handleMyPatientsNavigation = useCallback(() => {
    navigate('/doctor/patients');
  }, [navigate]);

  const handleMyScheduleNavigation = useCallback(() => {
    navigate('/doctor/doctor-calendar');
  }, [navigate]);

  const handlePrescriptionsNavigation = useCallback(() => {
            ...
            <div key={index} className="bg-white rounded-lg shadow p-4 md:p-6">
              <div className="flex items-center justify-between">
                ...
                <div className={`bg-${stat.color}-100 rounded-full p-2 md:p-3`}>
                  {stat.title.includes('Patient') ? <Users size={24} className={`text-${stat.color}-600`} /> : 
                   stat.title.includes('Appointment') ? <Calendar size={24} className={`text-${stat.color}-600`} /> :
                   stat.title.includes('Consultation') ? <Activity size={24} className={`text-${stat.color}-600`} /> :
    ...
    <p className="text-gray-500">Analytics data not available</p>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <button 
            onClick={handleMyPatientsNavigation}
          ...
          <button 
            onClick={handleMyScheduleNavigation}
          ...
          <button 
            onClick={handlePrescriptionsNavigation}
