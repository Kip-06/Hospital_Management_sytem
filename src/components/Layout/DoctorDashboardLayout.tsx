...
const DashboardLayout = () => {
  ...
  const userSpecialization = user.specialization || (user.role === 'doctor' ? 'Doctor' : user.role || '');
  ...
  const navLinks = [
    ...
    { name: 'Patient Updates', path: '/doctor/patient-updates', icon: <MessageSquare size={20} /> },
    { name: 'Medical Records', path: '/doctor/records', icon: <FileText size={20} /> },
    { name: 'Prescriptions', path: '/doctor/prescriptions', icon: <Clipboard size={20} /> },
    { name: 'Activities', path: '/doctor/activities', icon: <Activity size={20} /> },
    { name: 'Analytics', path: '/doctor/analytics', icon: <BarChart size={20} /> },
    { name: 'Calendar', path: '/doctor/doctor-calendar', icon: <Calendar size={20} /> },
      ...
      <div className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-white shadow-lg transition-all duration-300 ease-in-out overflow-hidden`}>
        <div className="flex items-center justify-between p-4 border-b">
          {isSidebarOpen ? (
            ...
            <h2 className="text-xl font-semibold text-blue-600">MC</h2>
          )}
            ...
            <div className="flex items-center mb-6">
              ...
              {isSidebarOpen && (
                <div className="ml-3">
                  ...
                  <p className="text-xs text-gray-500">{userSpecialization}</p>
                </div>
              )}
            </div>
          ...
          <div className="px-6 py-4 flex items-center justify-between">
            <h1 className="text-2xl font-semibold text-gray-800">
              {navLinks.find(link => 
                ...
                (link.path === '/doctor' && location.pathname === '/doctor/') ||
